...
class SearchView(OpenSearchView, ElasticSearchView):
    page_size = 10
    ...
    def get(self, request):
        ...
        self.context.update({'form': self.search_form,
        ...
        self.context.update(self.get_facet_labels(self.get_query()))
    ...
    def get_query(self):
        ...
        default_operator = cleaned_data.get('default_operator').upper()
        if default_operator not in ('AND', 'OR'):
            default_operator = 'AND'

        query = {
            'query': {'query_string': {'query': self.request.GET['q'],
                                       'default_operator': default_operator}},
            'from': cleaned_data['from'],
            'size': cleaned_data['page_size'],
        ...
        for key in list(self.request.GET):
            parameter = self.request.GET[key]
    ...
    def get_facet_labels(self, query):
        if not query:
            return {}
        ...
        for key in query['facets']:
            ...
            filter_value = self.request.GET.get('filter.%s' % query['facets'][key]['terms']['field'])
