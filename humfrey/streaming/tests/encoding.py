...
class CoercingIRIsTestCase(unittest.TestCase):
    bad_characters = '^<>"{}|^`\\' + ''.join(chr(i) for i in range(0, 33))

    def testBadCharacters(self):
        for char in self.bad_characters:
            original_iri = rdflib.URIRef('http://example.com/foo' + char + 'baz')
            expected_iri = rdflib.URIRef('http://example.com/foo%%%02Xbaz' % ord(char))
    ...
    def testGoodCharacters(self):
        original_iri = rdflib.URIRef('http://example.com/' + ''.join(chr(i) for i in range(0, 1024) if chr(i) not in self.bad_characters))
